...
exports.SecretStore = [
  ...
  function SStorage(initialNonce, initialHashedProof) {
      
      const storage = { "prim": "Pair", "args": [ [], { "prim": "Pair", "args": [ { "bytes": `${initialHashedProof}` }, { "int": `${initialNonce}` } ] } ] };
      return storage;
  }

  exports.SecretStoreStorage = SStorage;

  exports.SecretStoreMichelson = `parameter (pair (pair (string %encryptedData) (bytes %nexthashedProof)) (bytes %proof));
  ...
  function SMichelson(initialNonce, initialHashedProof) {

      const storage = `(Pair (Pair {} ${initialHashedProof}) ${initialNonce})`;
      return storage;
  }

  exports.SecretStoreMichelson = SMichelson;
  ...
  exports.credentials = {babylonnet: babylonnetCredentials, carthagenet: carthagenetCredentials}
